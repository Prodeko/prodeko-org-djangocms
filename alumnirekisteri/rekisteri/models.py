...
class Person(models.Model):
    ...
    #user = models.OneToOneField(User, related_name="person", null=True)
    ...
    show_student_organizational_activities_category = models.BooleanField(blank=False, default=False)
    ...
    def get_starting_year(self):
        e = self.get_tuta_educations().filter(~Q(degree_level=5)).exclude(
                   end_year__isnull=True
               )

        def sortByStartDate(x):
            return ((x.start_year if x.start_year else 0) * 12 + (x.start_month if hasattr(x, "end_month") and x.start_month else 0))
    ...
    def get_tuta_educations(self):
        return self.educations.filter(
               ...
               | Q(school__icontains='HUT')).filter(
    ...
    def get_family_members_sorted(self):
        return sorted(self.family_members.all(), key=lambda x: [3, 4, 0, 1, 2].index(x.member_type))
