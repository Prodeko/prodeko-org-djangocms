...
def sortByEndDate(x):
    return -((x.end_year if x.end_year else 0) * 12 + (x.end_month if hasattr(x, "end_month") and x.end_month else 0))
...
def getAdminSearchQueryset(first_name, last_name, admin_note, member_until_type, member_until, user_type):
    user_list = User.objects.all()

    if last_name:
        user_list = user_list.filter(Q(last_name__icontains=last_name) |
                                     Q(person__original_last_name__icontains=last_name))
    if first_name:
        user_list = user_list.filter(Q(first_name__icontains=first_name) |
...
def admin(request):
    ...
    if request.method == "POST" and "action" in request.POST:
        ...
        if request.POST.get("action") == "make-admin":
            ...
            return HttpResponseForbidden()
        if (request.POST.get("action") != "delete-user" and (user.last_login is None) and user.is_active):
...
def admin_export_matrikkeli(request):
    if request.GET.get('vuosikurssi', False):
        persons = Person.objects.all().filter(dont_publish_in_book=False).filter(
            is_alumni=True).order_by('class_of_year', 'user__last_name')
        response = HttpResponse(content_type='text/tex')
        response['charset'] = 'utf-8'
        response['Content-Disposition'] = 'attachment; filename="vuosikurssit.tex"'
        response.write(render_to_string('vuosikurssit.tex', {
            'persons': persons,
        }))
        return response

    persons = Person.objects.all().select_related().filter(
        is_alumni=True).order_by('user__last_name', 'user__first_name')
    category = "alumni"
    ...
    if not request.GET.get('images', False):
        ...
        response['Content-Disposition'] = 'attachment; filename="' + \
            category + '.tex"'
        response.write(render_to_string('matrikkeli.tex', {
            'persons': persons,
            'category': category
        }))
        return response
        ...
        response = StreamingHttpResponse(FileWrapper(
            open(path_to_zip, 'rb'), chunk_size), content_type='application/zip')
...
def admin_export_data(request):
    if request.method == "POST":
        ...
        queryset = getAdminSearchQueryset(request.POST.get('first_name_search'), request.POST.get('last_name_search'), request.POST.get('admin_note_search'),
                                          request.POST.get('member_until_type'), request.POST.get('member_until_search'), request.POST.get('member_type_search'))
        ...
        def make_row(u):
            ...
            def displayPosition(x):
                return (x.position + " @ " + x.organisation + " (" +
                        (str(x.start_year) if x.start_year else "") + "-" +
                        (str(x.end_year) if x.end_year else "") + ")")

            def displayEducation(x):
                return (x.field + ", " + (x.get_degree_level_display() if x.degree_level else "") + ": " +
                        (x.major if x.major else "") + (x.minor if x.minor else "") +
            ...
            if request.POST.get('educations'):
                row.append('§'.join(map(displayEducation, sorted(
                    p.educations.all(), key=sortByEndDate))))
            if request.POST.get('positions_of_trust'):
                row.append('§'.join(map(displayPosition, sorted(
                    p.positions_of_trust.all(), key=sortByEndDate))))
            if request.POST.get('student_organizational_activities'):
                row.append('§'.join(map(displayPosition, sorted(
                    p.student_organizational_activities.all(), key=sortByEndDate))))
            if request.POST.get('volunteers'):
                row.append('§'.join(map(displayPosition, sorted(
                    p.volunteers.all(), key=sortByEndDate))))
            if request.POST.get('honors'):
                row.append('§'.join(map(lambda x: x.title + ", " + x.organisation + (" (" + str(x.year) + ")" if x.year else ""),
            ...
            if request.POST.get('family_members'):
                row.append('§'.join(map(lambda x: x.first_name + " " + x.last_name +
                                        "(" + x.get_member_type_display() + ")", p.family_members.all())))
            return row
...
def admin_log(request):
    ...
    person_log = Person.audit_log.annotate(action_user_name=F('action_user__email')).annotate(
        target=Value('Person', output_field=models.CharField()))
    phone_log = Phone.audit_log.annotate(action_user_name=F('action_user__email')).annotate(
        target=Value('Phone', output_field=models.CharField()))
    email_log = Email.audit_log.annotate(action_user_name=F('action_user__email')).annotate(
        target=Value('Email', output_field=models.CharField()))
    skill_log = Skill.audit_log.annotate(action_user_name=F('action_user__email')).annotate(
        target=Value('Skill', output_field=models.CharField()))
    language_log = Language.audit_log.annotate(action_user_name=F('action_user__email')).annotate(
        target=Value('Language', output_field=models.CharField()))
    education_log = Education.audit_log.annotate(action_user_name=F(
        'action_user__email')).annotate(target=Value('Education', output_field=models.CharField()))
    work_experience_log = WorkExperience.audit_log.annotate(action_user_name=F(
        'action_user__email')).annotate(target=Value('WorkExperience', output_field=models.CharField()))
    position_of_trust_log = PositionOfTrust.audit_log.annotate(action_user_name=F(
        'action_user__email')).annotate(target=Value('PositionOfTrust', output_field=models.CharField()))
    student_organizational_activity_log = StudentOrganizationalActivity.audit_log.annotate(action_user_name=F(
        'action_user__email')).annotate(target=Value('StudentOrganizationalActivity', output_field=models.CharField()))
    volunteer_log = Volunteer.audit_log.annotate(action_user_name=F(
        'action_user__email')).annotate(target=Value('Volunteer', output_field=models.CharField()))
    honor_log = Honor.audit_log.annotate(action_user_name=F('action_user__email')).annotate(
        target=Value('Honor', output_field=models.CharField()))
    interest_log = Interest.audit_log.annotate(action_user_name=F('action_user__email')).annotate(
        target=Value('Interest', output_field=models.CharField()))
    family_member_log = FamilyMember.audit_log.annotate(action_user_name=F(
        'action_user__email')).annotate(target=Value('FamilyMember', output_field=models.CharField()))
    log_all = sorted(
        chain(
            person_log,
            phone_log,
        ...
        key=lambda entry: entry.action_date, reverse=True
    )
...
def admin_stats(request):
    ...
    dont_publish_in_book_logged_in = len(Person.objects.all().filter(
        dont_publish_in_book=True).exclude(user__last_login=None))
    ...
    show_student_organizational_activities_category = len(
        Person.objects.all().filter(show_student_organizational_activities_category=True))
    ...
    draw_dont_publish_in_book = str(
        math.ceil(dont_publish_in_book/total_count*100)) + "%"
    draw_dont_publish_in_book_logged = str(
...
def admin_set_notes(request):
    ...
    return render(request, 'admin_set_notes.html', {
...
def admin_edit_person_view(request, pk):
    ...
    person = user.person
    return edit_person(user, person, True, 'admin_edit_person.html', reverse('alumnirekisteri:admin_edit_person_view', args=[pk]), request)
...
def myprofile(request):
    user = request.user
    person = user.person
    return edit_person(user, person, False, 'myprofile/myprofile.html', reverse('alumnirekisteri:myprofile'), request)
...
def edit_person(user, person, adminview, template, form_action_url, request):
        ...
        if all([form.is_valid() for form in forms]):
            ...
            return JsonResponse({
                ...
                'errors': {
                    ...
                    'admin_form': dict(admin_form.errors.items()) if request.user.is_staff else None
                }
            })
    return render(request, template, {
        ...
        'work_experiences': sorted(person.work_experiences.all(), key=sortByEndDate),
        'positions_of_trust': sorted(person.positions_of_trust.all(), key=sortByEndDate),
        'student_organizational_activities': sorted(person.student_organizational_activities.all(), key=sortByEndDate),
...
def edit_phone(request, pk):
    ...
    return render(request, 'modals/edit_modal.html', {
        'title': 'Muokkaa',
        'form': form,
...
def search(request):
    ...
    if last_name:
        queryset = queryset.filter(Q(user__last_name__icontains=last_name) |
                                   Q(original_last_name__icontains=last_name))
    if first_name:
        queryset = queryset.filter(Q(user__first_name__icontains=first_name) |
...
def delete_profile(request):
    ...
    if request.method == "POST":
        if not (form.data["username"] == request.user.username or form.data["username"] == request.user.email):
