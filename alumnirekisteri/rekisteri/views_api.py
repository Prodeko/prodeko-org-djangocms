...
def modify_sheet(sheet_id, user):
    ...
    if values is None:
            ...
            if email in row:
                ...
                result = (
                    ...
                    .update(
                        ...
                        range=f"Sheet1!{row_num}:{row_num}",
        ...
        row = [
            ...
            user.first_name,
            user.last_name,
            person.get_member_type_display(),
            current_time,
        ]
        body = {"values": [row]}
        result = (
...
class Scanner(View):
    def post(self, request, *args, **kwargs):
                            ...
                            return JsonResponse(
                                {
                                    ...
                                    "member_type": user.person.get_member_type_display(),
                                    "message": f"Käyttäjä {user.email} kirjattu {direction}",
                                }
                            )
                        except HttpError as error:
                            print(f"An error occurred: {error}")
                            return JsonResponse(
                                {"active": False, "message": f"Tapahtui virhe: {error}"}
                            )
        ...
        return JsonResponse(
            {
                "active": False,
                "message": "Käyttäjä ei ole kirjautunut sisään, tai istunto on vanhentunut. Pyydä käyttäjää päivittämään sivu ja kirjautumaan sisään uudelleen.",
            }
        )
