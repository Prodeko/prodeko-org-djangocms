...
def latex_escape(value):
    return mark_safe(value.replace('&', '§§&').replace('#', '§§#').replace('_', '§§_').replace('\\', '§§textbackslash ').replace('§§', '\\'))
...
def sortByEndDate(x):
    return -((x.end_year if x.end_year else 10000 + (x.start_year if x.start_year else 0)) * 12 + (x.end_month if hasattr(x, "end_month") and x.end_month else 0))
def sortByYear(x):
...
def person_is_empty(person):
    return not (person.birthdate or person.address or person.postal_code or person.city or person.gender or person.marital_status or person.military_rank or person.class_of_year or person.phones.all() or person.educations.all() or person.work_experiences.all() or  person.positions_of_trust.all() or person.student_organizational_activities.all() or person.volunteers.all() or person.honors.all() or person.interests.all() or person.languages.all() or person.languages.all() or person.get_family_members_sorted())
